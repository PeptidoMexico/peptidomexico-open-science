...
const WATER = { C: 0, H: 2, N: 0, O: 1, S: 0 };
...
const MONOISOTOPIC_ATOMIC_MASS = {
    C: 12,
    H: 1.00782503223,
    N: 14.00307400443,
    O: 15.99491461957,
...
const POSITIVE_PKA = { N_TERMINUS: 8.6, K: 10.5, R: 12.5, H: 6.0 };
const NEGATIVE_PKA = { C_TERMINUS: 3.6, D: 3.9, E: 4.1, C: 8.3, Y: 10.1 };
...
function validateModification(modification) {
    if (!modification.name.trim())
        throw new RangeError("Modification name must not be empty.");
...
function modificationDeltaMass(modification) {
    if (modification.deltaFormula !== undefined)
        return formulaMass(modification.deltaFormula, "monoisotopic");
    return modification.deltaMassDa ?? 0;
}
function formulaMass(formula, mode) {
...
export function normalizeSequence(sequence) {
    ...
    if (normalized.length === 0)
        throw new RangeError("sequence must contain at least one standard amino acid.");
...
export function calculateComposition(sequence) {
    ...
    const composition = Object.fromEntries(AMINO_ACID_CODES.map((code) => [code, 0]));
    for (const code of normalized)
        composition[code] += 1;
    return composition;
}
export function calculatePeptideFormula(sequence, modifications = []) {
    const normalized = normalizeSequence(sequence);
    validateModifications(modifications);
    const formula = { C: 0, H: 0, N: 0, O: 0, S: 0 };
    for (const code of normalized)
        addFormula(formula, AMINO_ACIDS[code].residueFormula);
    addFormula(formula, WATER);
    for (const modification of modifications) {
        if (modification.deltaFormula !== undefined)
            addFormula(formula, modification.deltaFormula);
    }
    for (const element of ELEMENTS) {
        if (formula[element] < 0)
            throw new RangeError(`Calculated formula has a negative ${element} count.`);
    }
    return formula;
}
...
export function calculateModificationDeltaMass(modifications = []) {
    validateModifications(modifications);
    return modifications.reduce((sum, modification) => sum + modificationDeltaMass(modification), 0);
}
export function calculatePeptideMass(sequence, modifications = []) {
...
function fractionProtonatedBase(pH, pKa) {
    return 1 / (1 + 10 ** (pH - pKa));
}
function fractionDeprotonatedAcid(pH, pKa) {
    return 1 / (1 + 10 ** (pKa - pH));
}
export function calculateNetCharge(sequence, pH = 7) {
    const normalized = normalizeSequence(sequence);
    assertFinite(pH, "pH");
    if (pH < 0 || pH > 14)
        throw new RangeError("pH must be between 0 and 14 for this approximation.");
...
export function calculateIsoelectricPoint(sequence) {
    ...
    for (let iteration = 0; iteration < 80; iteration += 1) {
        ...
        if (calculateNetCharge(sequence, midpoint) > 0)
            low = midpoint;
        else
            high = midpoint;
    }
...
export function calculateHydropathy(sequence, windowSize = 7) {
    const normalized = normalizeSequence(sequence);
    ...
    const residueValues = [...normalized].map((code) => AMINO_ACIDS[code].kyteDoolittle);
...
export function analyzePeptide(sequence, options = {}) {
    const normalized = normalizeSequence(sequence);
