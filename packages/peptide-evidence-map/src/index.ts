...
export interface EvidenceMap {
  ...
  duplicateGroups: Array<{ key: string; recordIds: string[] }>;
  summary: {
    inputRecords: number;
...
function cleanText(value: string, field: string): string {
  ...
  if (!cleaned) throw new RangeError(`${field} must not be empty.`);
  return cleaned;
}

function validateYear(year: number | undefined): number | undefined {
...
function validateDate(value: string | undefined, field: string): string | undefined {
  ...
  if (!/^\d{4}-\d{2}-\d{2}$/.test(cleaned)) throw new RangeError(`${field} must use YYYY-MM-DD.`);
...
function normalizeUrl(value: string): string {
  ...
  if (!["http:", "https:"].includes(url.protocol)) throw new RangeError("URL identifiers must use http or https.");
  url.hash = "";
  url.search = "";
...
function normalizeIdentifierValue(input: string | IdentifierInput): NormalizedIdentifier {
  ...
  if (explicitType === "doi") {
    ...
    if (!/^10\.\d{4,9}\/\S+$/.test(normalized)) throw new RangeError("Invalid DOI identifier.");
    return { type: "doi", value: normalized, key: `doi:${normalized}` };
  }
  if (explicitType === "pmid") {
    const normalized = value.replace(/^pmid:/i, "");
    if (!/^\d+$/.test(normalized)) throw new RangeError("PMID identifiers must contain digits only.");
  ...
  if (explicitType === "pmcid") {
    ...
    if (!/^PMC\d+$/.test(normalized)) throw new RangeError("PMCID identifiers must use the PMC123 format.");
  ...
  if (pmidMatch) return { type: "pmid", value: pmidMatch[1], key: `pmid:${pmidMatch[1]}` };
...
export function normalizeIdentifier(input: string | IdentifierInput): NormalizedIdentifier {
...
function hashRecords(records: NormalizedEvidenceRecord[]): string {
  return createHash("sha256").update(stableJson(records)).digest("hex");
}

function normalizeRecord(record: EvidenceInputRecord, index: number, options: EvidenceMapOptions): NormalizedEvidenceRecord {
  ...
  if (!EVIDENCE_TIERS.includes(evidenceTier)) throw new RangeError(`Unsupported evidenceTier: ${evidenceTier}.`);
...
export function ingestEvidence(records: EvidenceInputRecord[], options: EvidenceMapOptions = {}): EvidenceMap {
  if (!Array.isArray(records) || records.length === 0) throw new RangeError("records must contain at least one record.");
  ...
  for (const record of normalized) groups.set(record.identifier.key, [...(groups.get(record.identifier.key) ?? []), record.id]);
  ...
  const byIdentifierType: Partial<Record<IdentifierType, number>> = {};
  const byEvidenceTier: Partial<Record<EvidenceTier, number>> = {};
...
export function parseEvidenceCsv(csv: string): EvidenceInputRecord[] {
  const lines = csv.split(/\r?\n/).map((line) => line.trim()).filter(Boolean);
  if (lines.length < 2) throw new Error("CSV must include a header and at least one record.");
  ...
  return lines.slice(1).map((line, index) => {
    ...
    if (fields.length !== expected.length) throw new Error(`CSV line ${index + 2} has the wrong number of fields.`);
    const year = fields[4] ? Number(fields[4]) : undefined;
    if (fields[4] && !Number.isInteger(year)) throw new Error(`CSV line ${index + 2} has an invalid year.`);
    return {
      ...(fields[0] ? { id: fields[0] } : {}),
      identifier: { type: (fields[1] || undefined) as IdentifierType | undefined, value: fields[2] },
      ...(fields[3] ? { title: fields[3] } : {}),
      ...(year === undefined ? {} : { year }),
      ...(fields[5] ? { evidenceTier: fields[5] as EvidenceTier } : {}),
