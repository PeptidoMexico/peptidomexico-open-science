...
import { ingestEvidence, parseEvidenceCsv, type EvidenceInputRecord } from "./index.js";
...
function main(): void {
  ...
  if (command !== "ingest") throw new Error(`Unknown command: ${command}.`);
  ...
  let records: EvidenceInputRecord[];
  if (format === "csv") {
    records = parseEvidenceCsv(text);
  } else if (format === "json") {
    const parsed = JSON.parse(text) as EvidenceInputRecord[] | { records: EvidenceInputRecord[] };
    records = Array.isArray(parsed) ? parsed : parsed.records;
  } else {
    throw new Error("--format must be json or csv.");
  }
  if (!Array.isArray(records)) throw new Error("Input JSON must be an array or an object with a records array.");
